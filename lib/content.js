    ...
    function getContent(path, conf, callback){
        conf.github.ghrepo.contents(path, function(err, data){
            if(err){
                ...
                if(data.type === 'file'){
                    var rawHeader = { Accept: 'application/vnd.github.v3.raw+json'
                                    , 'User-Agent': 'patchwork-thread (https://github.com/joebadmo/patchwork) terminal/0.0'
                                    };
                    ...
                    console.log("using rawHeader",rawHeader);
                    console.log("using rawPath",rawPath);
                    var options = {
                        uri: rawPath,
                        headers: rawHeader
                    };
